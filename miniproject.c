...
void intro()
{
    ...
    printf("This system allows you to sell crops,hire labours,knowledge on farming.");
...
void farmer()
{
    ...
    if(confirm=10)
    {
        ...
        else if(op==2)
        {
            ...
            if(count=0)
            {
                printf("\nNO LABOURS ARE AVAILABLE");
            }
            else
            {
                printf("\n%d LABOURS ARE AVAILABLE");
...
void buy()
{
	...
	int found=0;
    FILE *ft;
	ft=fopen("input.dat","w+");
	printf("\n Enter name of the crop to buy: ");
	scanf("%s",name);
	printf("\nEnter the weight in quintals:");
	scanf("%d",&we);
	printf("\nEnter the rate per quintal:");
	scanf("%d",&qpr);
	name[0]=toupper(name[0]);
	while(fscanf(ft,"%s %d %d\n", items.crop,items.we,items.qpr)!=EOF)
    {
        if (strcmp(items.crop,name)!=0)
        {
            found=0;
        }
        else
...
void login()
{
    ...
    char user[15];
    char pass[15];
    char original_user[25]="farmer";
    char original_pass[15]="ilovefarming";
    do
    {
        printf("\n\nEnter you USER NAME:");
        scanf("%s",user);
